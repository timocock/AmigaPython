...
/* Add in INITFUNC() near the end */
#ifdef _AMIGA
    /* Initialize Amiga-specific functionality */
    if (PyDict_SetItemString(d, "O_BINARY", PyInt_FromLong(0)) < 0)
        return;
    if (PyDict_SetItemString(d, "O_NOINHERIT", PyInt_FromLong(0)) < 0)
        return;
#endif
